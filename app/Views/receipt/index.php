...
<body>
    ...
    <div class="toolbar no-print flex flex-wrap items-center justify-between gap-2">
        <a href="<?php echo url($isTenantPortal ? 'tenant-payments' : 'payments'); ?>" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left mr-1 inline-block"></i><?php echo t('back'); ?></a>
        <div class="toolbar-actions w-full md:w-auto flex flex-wrap items-center gap-2">
            <?php if ($currentLang === 'bn'): ?>
                <a class="btn btn-outline-dark btn-sm btn-tool" href="<?php echo url('receipt') . '?id=' . (int)$receipt['id'] . '&lang=en' . $copySuffix; ?>"><i class="bi bi-translate mr-1"></i>English</a>
            <?php else: ?>
                <a class="btn btn-outline-dark btn-sm btn-tool" href="<?php echo url('receipt') . '?id=' . (int)$receipt['id'] . '&lang=bn' . $copySuffix; ?>"><i class="bi bi-translate mr-1"></i>বাংলা</a>
            <?php endif; ?>
            <?php if ($isTenantPortal): ?>
                <!-- Tenant sees only tenant copy (forced) -->
            <?php else: ?>
                <?php if ($copyType === 'original'): ?>
                    <a class="btn btn-outline-warning btn-sm btn-tool" href="<?php echo url('receipt') . '?id=' . (int)$receipt['id'] . '&' . ($currentLang === 'bn' ? 'lang=bn' : 'lang=en') . '&copy=tenant'; ?>"><i class="bi bi-files mr-1"></i><?php echo $currentLang === 'bn' ? 'ভাড়াটিয়া কপি' : 'Tenant Copy'; ?></a>
                <?php else: ?>
                    <a class="btn btn-outline-secondary btn-sm btn-tool" href="<?php echo url('receipt') . '?id=' . (int)$receipt['id'] . '&' . ($currentLang === 'bn' ? 'lang=bn' : 'lang=en'); ?>"><i class="bi bi-file-earmark mr-1"></i><?php echo $currentLang === 'bn' ? 'মূল কপি' : 'Original'; ?></a>
                <?php endif; ?>
            <?php endif; ?>
