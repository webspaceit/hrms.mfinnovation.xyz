...
// Style matching rms.webspaceit.com. Former receipt.php, now a
// controller + view. Serves BOTH the admin panel and the tenant
// portal (branches on $isTenantPortal).
// ============================================================
    ...
    public function index(): void {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'upload_signature') {
            ...
            if ($isTenantPortal || !csrf_verify()) {
                ...
                redirect($isTenantPortal ? 'tenant-payments' : 'payments');
            }
        ...
        $sigSrc = (!empty($receipt['signature']) && file_exists(dirname(__DIR__) . '/' . $receipt['signature']))
            ? BASE_URL . $receipt['signature']
            : $ownerSig;

        $this->view('receipt/index', [
