...
<div class="filter-bar mb-3 fade-in flex flex-wrap items-center gap-2">
    ...
    <select class="form-select form-select-sm" style="max-width: 180px;" onchange="location='payments.php?month='+this.value+'&year=<?php echo $year; ?>'">
        <option value="0"><?php echo t('all'); ?></option>
        <?php for ($m = 1; $m <= 12; $m++): ?>
            <option value="<?php echo $m; ?>" <?php echo $month == $m ? 'selected' : ''; ?>><?php echo monthName($m); ?></option>
        <?php endfor; ?>
    </select>
    <select class="form-select form-select-sm" style="max-width: 130px;" onchange="location='payments.php?month=<?php echo $month; ?>&year='+this.value">
                                    ...
                                    if ($isDue) {
                                        echo actionButtons([
                                            ...
                                            ['label' => t('view_invoice'), 'icon' => 'bi bi-eye', 'variant' => 'view',
                                             'href' => BASE_URL . 'invoice_view.php?id=' . (int)$p['invoice_id'] . '&lang=' . Lang::current()],
                                        ]);
                                    } else {
                                        $payItems = [
                                        ...
                                        $payItems[] = ['label' => t('money_receipt'), 'icon' => 'bi bi-receipt', 'variant' => 'view', 'href' => BASE_URL . 'receipt.php?id=' . (int)$p['id']];
