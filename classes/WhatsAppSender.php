<?php
// ============================================================
// WhatsAppSender - generic HTTP WhatsApp Business API gateway
...
class WhatsAppSender {
    ...
    public static function send($phone, $message) {
        $url = trim((string)getSetting('wa_api_url', ''));
        $key = trim((string)getSetting('wa_api_key', ''));
        if ($url === '' || $key === '') {
            return ['success' => false, 'response' => null, 'error' => t('wa_not_configured')];
        }

        $params = [
            'api_key' => $key,
            'sender_id' => trim((string)getSetting('wa_sender_id', '')),
